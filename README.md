...
- 由于移动端界面登录的问题，目前该插件只能在电脑端浏览器使用，但适配移动端edge浏览器的版本将在不久后推出
- 当前该脚本可以服务于日常使用电脑的同学，你可以在常用浏览器上进行安装
- **移动端版本**的篡改猴脚本推出后，你将真正实现日常刷刷手机即可自动获取最新劳动开课信息
> 东南大学劳动教育选课神器！  
...
- 由于浏览器会对后台标签页进行性能限制或挂起（如 Chrome 的 Tab Freezing），脚本的“报活”机制可能失效，导致重复打开新的选课标签页。这是浏览器节能策略所致，并非脚本错误。目前延长了增加冷却时间和检查间隔后，似乎没再出现过这个问题。
